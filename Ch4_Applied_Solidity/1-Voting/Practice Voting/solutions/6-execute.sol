// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

contract Voting {
    mapping(address => bool) public authorizedMembers;
    constructor(address[] memory members) {
        authorizedMembers[msg.sender] = true;
        for (uint i = 0; i < members.length; i++) {
            authorizedMembers[members[i]] = true;
        }
    }

    struct Proposal {
        address target;
        bytes data;
        uint yesCount;
        uint noCount;
    }

    Proposal[] public proposals;
    mapping(uint => mapping(address => bool)) public votes;
    mapping(uint => mapping(address => bool)) public hasVoted;

    event ProposalCreated(uint proposalId);
    event VoteCast(uint proposalId, address voter);
    event ProposalExecuted(uint proposalId);

    modifier onlyAuthorized() {
        require(authorizedMembers[msg.sender], "Not an authorized member");
        _;
    }

    function newProposal(
        address target,
        bytes calldata data
    ) external onlyAuthorized {
        Proposal memory initialProposal = Proposal({
            target: target,
            data: data,
            yesCount: 0,
            noCount: 0
        });

        proposals.push(initialProposal);
        emit ProposalCreated(proposals.length - 1);
    }

    function castVote(uint proposalId, bool vote) external onlyAuthorized {
        if (hasVoted[proposalId][msg.sender]) {
            bool prevVote = votes[proposalId][msg.sender];

            if (vote != prevVote) {
                if (prevVote) {
                    proposals[proposalId].yesCount -= 1;
                    proposals[proposalId].noCount += 1;
                } else {
                    proposals[proposalId].yesCount += 1;
                    proposals[proposalId].noCount -= 1;
                }
                votes[proposalId][msg.sender] = vote;
            } else {
                if (prevVote) {
                    proposals[proposalId].yesCount -= 1;
                } else {
                    proposals[proposalId].noCount -= 1;
                }
                hasVoted[proposalId][msg.sender] = false;
            }
        } else {
            if (vote == true) {
                proposals[proposalId].yesCount += 1;
            } else {
                proposals[proposalId].noCount += 1;
            }
            hasVoted[proposalId][msg.sender] = true;
            votes[proposalId][msg.sender] = vote;
        }

        emit VoteCast(proposalId, msg.sender);

        if (proposals[proposalId].yesCount >= 10) {
            Proposal storage proposal = proposals[proposalId];
            (bool success, ) = proposal.target.call(proposal.data);
        }
    }
}
